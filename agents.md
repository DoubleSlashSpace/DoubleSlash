...
- Preserve root/device TLS claim checks, separately tracked direct/relay routes, sibling-safe replacement/disconnect, per-endpoint room subscriptions, and one-answer call selection. An endpoint that loses the selection gets `CallAnsweredElsewhere`, not `CallEnded`: it stops ringing (the desktop closes its dialog without sending a reject) and must not count a missed call. Android maps it to the existing `call_ended` wire event. A stale disconnect must not delete a replacement or sibling route.
...
- File-based device moves still require quitting the source before connecting the destination. Same-identity acceptance targets the normal packaged desktop profile and phone, not the historical `.clientA` test identity. Connected, room-chat, reconnect, third-contact and relay checks passed on that setup on 2026-09-22; do not claim installed simultaneous-use support until call answering on either device and the restore edge cases in the backlog also pass. See [Devices and backups](docs/DEVICES_AND_BACKUPS.md).
...
- Do not reintroduce cluster `RoomGrant`/private allowed-set replication. Cluster links propagate room rosters, Space roots, peer trust, and opaque room frames. Replicated chat, audio and camera on/off state (`SfuVideoState`) must be deduplicated and never re-replicated.
...
- Room offers carry purpose `room_file` from every client; the desktop receiver files any other purpose as a 1:1 file from the sender. Record our own offer under `room_conversation_id`, the key `room.history` reads.
- Room file frames are delivered per device, like room chat: to every room endpoint except the sending device, including the author's other devices (`file_frame_endpoints`). A `to` frame goes only to that identity's endpoints, so a desktop can pull from its own phone. `FileOffered.is_self` means *this device's* offer; a sibling's offer is inbound and downloadable. File the bubble under the room's sidebar node (`room_file_node`), not the delivering cluster sibling.
- The Ollama `send_file` tool may only reach history attachments and canonicalized paths inside `ollama_share_folder` (`ollama_share.rs`). Keep the real-path containment check, the refusal of folders overlapping the profile, and the per-call settings check. Do not add fuzzy or "latest" fallbacks when choosing a file to send.
