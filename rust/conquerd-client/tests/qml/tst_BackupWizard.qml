...
ApplicationWindow {
    ...
    BackupWizard { id: wizard; appBackend: backend }
    ...
    TestCase {
        ...
        function test_locked_profiles_and_preview() {
            ...
            backend.finish({ok: true, profiles: [{profile: "original", public_id: "example-identity"}]})
            ...
            backend.finish({ok: true, token: "preview-token", summary: {public_id: "example-identity", messages: 12, attachments: 2, missing_attachments: 1}})
            ...
            backend.finish({ok: true, summary: {messages: 12, attachments: 2}})
        ...
        function test_export_errors_allow_retry() {
            ...
            action.clicked()
            backend.finish({ok: false, error: "Disk full"})
            compare(wizard.page, "export")
            compare(wizard.errorText, "Disk full")
            tryCompare(action, "enabled", true)
            action.clicked()
            compare(wizard.errorText, "")
            backend.finish({ok: true, summary: {messages: 12, attachments: 2, missing_attachments: 0}})
            compare(wizard.page, "done")
            compare(password.text, "")
