...
#[derive(Parser, Debug)]
#[command(name = "doubleslash-installer", version, about)]
struct Cli {
    ...
    #[arg(long, default_value = "DoubleSlashSpace/DoubleSlash")]
    repo: String,
...
fn default_install_dir() -> PathBuf {
    ...
    base.join(brand::WINDOWS_INSTALL_DIR)
}

/// Windows client archives published by our build scripts:
/// `DoubleSlash-<version>-win64.7z`.
...
fn run_silent(
    ...
    // Try to detect version from archive filename (DoubleSlash-1.2.3-win64.7z)
...
/// Try to extract a version string from an archive filename like `DoubleSlash-1.2.3-win64.7z`.
...
mod tests {
    ...
    fn launchable_current_dir_requires_current_executable() {
        ...
        let ver_dir = tmp.path().join("doubleslash_1.0.0");
        ...
        std::fs::write(ver_dir.join("DoubleSlash.exe"), b"test").expect("exe marker");
