...
//
// Games load inside the native client (`d://` + window.doubleslash).
// Datagrams ride the authenticated QUIC relay via portal channel APIs
// (`/_doubleslash/channel/*`). There is no WebTransport / self-signed TLS path.
//
// Wire (relay):
 ...
 * `doubleslash_features::channel_frame`.
 *
 * @readonly
 * @enum {number}
 */
 ...
 *   .disconnect()
 *
 * Transport is the native portal bridge (`window.doubleslash`).
 */
export class DoubleSlashClient {
    ...
    async connect() {
        const bridge = (typeof window !== "undefined") && window.doubleslash;
        if (!bridge?.ready) {
            const err = new Error(
