...
pub struct SignalingMessage {
    ...
    /// Endpoint locators authenticated by the enclosing identity signature.
...
impl SignalingMessage {
    ...
    pub fn canonical_bytes(&self) -> Vec<u8> {
        ...
        self.insert_device_addresses(&mut map);
    ...
    fn insert_device_addresses(&self, map: &mut serde_json::Map<String, Value>) {
        for (name, device) in [
            ...
            if let Some(device) = device {
                map.insert(
                    name.into(),
                    Value::Array(device.0.into_iter().map(Value::from).collect()),
                );
            }
        }
    }
    ...
    pub fn from_json(raw: &str) -> Result<Self, serde_json::Error> {
        ...
        let source_device = obj
            ...
            .transpose()?
            .flatten();
        let target_device = obj
            .get("target_device")
            .cloned()
            .map(serde_json::from_value)
            .transpose()?
            .flatten();
        if target_device.is_some() && target.is_none() {
...
mod tests {
    ...
    fn device_canonical_bytes_match_shared_wire_fixture() {
        ...
        assert_eq!(message.canonical_bytes(), fixture.as_bytes());
    }

    #[test]
    fn device_addresses_are_signed_and_cannot_be_retargeted() {
        ...
        assert!(!message.to_json().contains("device"));
        ...
        let signed = message.sign(&identity);
        let parsed = SignalingMessage::from_json(&signed.to_json()).unwrap();
    ...
    #[test]
    fn device_address_parser_rejects_bad_lengths_and_missing_identity() {
        assert!(SignalingMessage::from_json(
            r#"{"type":"ping","sender":"sender","source_device":[1]}"#
        )
        .is_err());
        let mut message = SignalingMessage::new(MessageType::Ping, "sender", serde_json::json!({}));
        message.target_device = Some(doubleslash_features::DeviceId([2; 32]));
        assert!(SignalingMessage::from_json(&message.to_json()).is_err());
    }
