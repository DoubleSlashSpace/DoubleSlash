...
///
/// A supernode logs an https invite (`https://doubleslash.space/i#…`);
/// `doubleslash://` and `d://` hand-off forms are matched too. Mirrors
/// `doubleslash_features::find_app_url`, which this crate's workspace cannot
/// depend on.
...
mod tests {
    ...
    fn extracts_invite_url_from_logs_text() {
        let text =
            "source: /var/lib/doubleslash/a/reusable_invite.json\n\ndoubleslash://invite#abc123\n";
        assert_eq!(
            extract_invite_url(text).as_deref(),
            Some("doubleslash://invite#abc123")
        );
