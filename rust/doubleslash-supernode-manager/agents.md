...
A standalone Rust CLI + TUI for deploying and operating fleets of `doubleslash-supernode` instances over SSH. It lives at `rust/doubleslash-supernode-manager/` in the DoubleSlash repository, in its own Cargo workspace; it is independent of the application crates but targets the same supernode release artifacts.
...
- Cache the extracted binary and all `licenses/` files locally; upload notices
  before promoting the binary on `install`. Missing distribution notices block
  installation. Adjacent notices use `<binary>.licenses/` locally and remotely.
...
- `version = "local"` skips download; operator must set `defaults.binary_path`
  and supply its adjacent `.licenses/` directory. `build-deploy` generates that
  directory using Node and cargo-about 0.8.4 before deployment.
...
| Manager-generated `access_mode` / `identity_file` runtime application | Gap — fields are serialized but not applied by `doubleslash-supernode` |
...
# or
.\launch.ps1 build-deploy --host acdc --all --source ..\doubleslash-supernode    # compile local source
...
.\launch.ps1 exec --host acdc --instance a "systemctl status doubleslash-supernode@a --no-pager"
```
