...
use doubleslash_client::protocol::{MessageType, SignalingMessage};
...
pub fn dispatch(session: &Session, request: &str) -> Value {
    ...
    match cmd.as_str() {
        ...
        "identity.export_key" => json!({
            "ok": true,
            "key": doubleslash_client::crypto::b64url_encode(&session.identity_key),
        }),
            ...
            let my_peer_id = session.identity.peer_id();
            {
                ...
                match store.get_mut(&my_peer_id) {
                    ...
                    None => {
                        ...
                        let mut record = doubleslash_client::peer_store::PeerRecord {
        ...
        "avatar.set_config" => {
            ...
            let config: doubleslash_client::avatar_config::AvatarConfig =
        ...
        "avatar.svg" => {
            ...
            let seed = doubleslash_client::avatar_config::avatar_seed_id(
                raw_id,
                &my_public_id,
                &my_peer_id,
                |raw| {
            ...
            json!({
                "ok": true,
                "svg": doubleslash_client::avatar_config::build_avatar_svg(&seed, &config),
                "tint": doubleslash_client::avatar_config::avatar_tint_hex(&seed, &config),
            })
        }
        ...
        "room.list" => {
            ...
            let entries: Vec<doubleslash_client::room_store::RoomEntry> = if members.is_empty() {
        ...
        "room.join" => {
            ...
            let token = entry
                ...
                .unwrap_or_default();
            let use_invite = doubleslash_client::connection_manager::should_use_private_room_invite(
                false,
        ...
        "room.history" => {
            ...
            let key = doubleslash_client::chat_store::room_conversation_id(room_id);
        ...
        "portal.send" => {
            ...
            let Ok(payload) = doubleslash_client::crypto::b64url_decode(payload_b64) else {
        ...
        "portal.poll" => {
            ...
            let frames: Vec<Value> = {
                ...
                queue
                    .drain(..)
                    .map(|payload| {
                        Value::String(doubleslash_client::crypto::b64url_encode_nopad(&payload))
                    })
                    .collect()
            };
        ...
        "audio.set_muted" => {
            ...
            queued(
                session
                    .call_tx
                    .try_send(doubleslash_client::call_controller::CallCommand::SetMuted(
                        muted,
                    ))
                    .is_ok(),
            )
...
fn space_invite_fields(session: &Session, supernode_id: &str, room_id: &str) -> (String, String) {
    let space_id = doubleslash_client::room_store::RoomStore::space_id_for(
