...
pub const DEFAULT_KEY_DIR_SUFFIX: &str = doubleslash_features::DEFAULT_PROFILE_DIR;
...
impl Identity {
    ...
    /// Default key directory: `~/.doubleslash`.
    ///
    /// Resolution order: `DOUBLESLASH_KEY_DIR` / `DOUBLESLASH_HOME` →
    /// `~/.doubleslash`.
    ...
    pub fn default_profile_root() -> PathBuf {
        if let Some(path) = doubleslash_features::first_env(&[
            doubleslash_features::ENV_KEY_DIR,
            doubleslash_features::ENV_HOME,
        ]) {
...
mod tests {
    ...
    fn default_key_dir_honors_profile_env() {
        ...
        std::env::set_var("DOUBLESLASH_KEY_DIR", &key_dir);
