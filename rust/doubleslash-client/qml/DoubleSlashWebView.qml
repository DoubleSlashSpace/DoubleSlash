...
Item {
    ...
    /// When true, navigation to d:// and doubleslash:// URLs is permitted.
    ...
    function isPortalUrl(url) {
        return /^(d|doubleslash):/i.test(url.toString())
    }

    function browserUrl(url) {
        // Chromium's Windows URL fixup interprets a one-letter scheme as a
        // drive even though Qt registered it. Rewrite `d://` to the long
        // form before handing the URL to WebEngine; preserve its remainder.
        return url.toString().replace(/^d:\/\//i, "doubleslash://")
    }
    ...
    WebEngineView {
        ...
        onNavigationRequested: function(request) {
            ...
            if (scheme === "d" || scheme === "doubleslash") {
